...
import { SfuClient, getActiveSfuClient, setActiveSfuClient } from '@/lib/mediasoup-client';
...
import { preloadAllAsBlob, preloadAsBlob } from '@/lib/image-cache';
...
export default function GroupPage() {
  ...
  // En la página de grupo, siempre volvemos al tab de grupos en AppHome
  ...
  async function handleLeaveGroup() {
    if (leaveBusy) return;
    if (!confirm('┬┐Abandonar el grupo? Dejar├ís de ser miembro y no aparecer├í en tu lista.')) return;
    setLeaveBusy(true);
    try {
  ...
  async function handleHardDeleteGroup() {
    ...
    if (!confirm('ΓÜá∩╕Å ┬┐Eliminar el grupo PERMANENTEMENTE? Esta acci├│n NO se puede deshacer. Se borrar├í el grupo, todos sus canales, mensajes, miembros, registros y archivos.')) return;
    if (prompt('Escribe exactamente "ELIMINAR PERMANENTEMENTE" para confirmar:') !== 'ELIMINAR PERMANENTEMENTE') {
      setFeedback('Eliminaci├│n cancelada: no escribiste la confirmaci├│n correctamente.');
      return;
    }
  ...
  // Preload voice participant avatars as blob: URLs so images appear instantly
  // when avatar grid renders — no flash, no bounce.
  useEffect(() => {
    const urls = voiceParticipants
  ...
  useEffect(() => {
    void loadGroup();
    // Preload self-avatar as blob on initial mount to prevent flicker on F5/re-mount
    if (user?.avatarUrl) {
      void preloadAsBlob(resolveMediaUrl(user.avatarUrl));
    }
  }, [groupId, user?.avatarUrl]); // Add user?.avatarUrl to dependencies
  ...
  useEffect(() => {
    ...
    const onChannelUpdated = (channel: Channel) => {
      ...
      if ((channel.type === 'VOICE' || channel.type === 'VIDEO') && !channel.isEnabled) {
        ...
        getActiveSfuClient()?.disconnect().catch(() => undefined);
  ...
  const voiceHeroMembers = useMemo(
    () =>
      voiceParticipants
        .map((participant) => ({
          id: participant.id,
  ...
  useEffect(() => {
    ...
    const onApproved = async (payload: { channelId: string }) => {
      ...
      try {
        await getActiveSfuClient()?.disconnect().catch(() => undefined);
        setActiveSfuClient(null);
        const sfu = new SfuClient(voiceChannel.id, user?.id);
        await sfu.connect();
        setActiveSfuClient(sfu);
        setVoiceJoined(true);
        setVoiceRequestPending(false);
    ...
    return () => {
      mounted = false;
      // DO NOT unsubscribe from voice_state_changed or voice_request_approved
      // These listeners must survive page unmount so the voice state (avatar, participants)
      // stays synchronized from the database via socket.io, enabling idempotent updates
      // even when the group page is unmounted (e.g., when using the VoiceOverlay bubble)
  ...
  // ΓöÇΓöÇ Listen for leave request from VoiceOverlay (kept as fallback for when page IS mounted) ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Register leave callback in Zustand store so VoiceOverlay can trigger leave
  //     even when the group page is unmounted ΓöÇΓöÇ
  useEffect(() => {
    voiceStoreSetOnLeaveRequested(() => {
      handleVoiceLeaveRef.current();
    });
    // NO cleanup ΓÇö the callback MUST survive unmount because the VoiceOverlay
    // bubble is only visible when this page is unmounted.
    // On re-mount the effect re-runs and overwrites with the new callback.
  }, [voiceStoreSetOnLeaveRequested]);

  // ΓöÇΓöÇ Listen for mute toggle from VoiceOverlay (fallback for when page IS mounted) ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Register mic-toggle callback in Zustand store so VoiceOverlay can control
  //     the real SFU mic even when the group page is unmounted ΓöÇΓöÇ
  useEffect(() => {
  ...
  // ΓöÇΓöÇ Speaking detection sync for the local user ΓöÇΓöÇ
  useEffect(() => {
    const activeSfu = getActiveSfuClient();
    if (!activeSfu || !voiceJoined || !user) return;
    activeSfu.onSpeakingChange = (isSpeaking: boolean) => {
      setVoiceParticipants((prev) =>
        prev.map((p) => (p.id === user.id ? { ...p, isSpeaking: isSpeaking } as any : p)),
      );
    };
    return () => { if (activeSfu) activeSfu.onSpeakingChange = undefined; };
  }, [voiceJoined, user?.id]);
  ...
  // Keep speaker connection alive when leaving the page so the avatar persists.
  // The SFU connection stays open and the mic is just paused (track.enabled = false).
  // The server sees the user as a speaker but with no audio being sent.
  // When the user re-enters, they're still in speaker mode ready to talk.
  useEffect(() => {
    return () => {
      // Cleanup intentionally empty — keep the speaker connection alive
      // so the avatar stays in the participant list and the bubble can control the mic
    };
  }, []);
  ...
  // "user is joined" would re-run this effect ΓåÆ cancel the in-flight listener
  // connection ΓåÆ user stuck with voiceJoined=true & sfuRef=null (race condition).
  useEffect(() => {
    if (!voiceChannel || !voiceChannel.isEnabled) return;
    // If the global SfuClient is still alive from the bubble, keep using it.
    const existingSfu = getActiveSfuClient();
    if (existingSfu) {
      if (existingSfu.channelId === voiceChannel.id) return; // bubble kept it alive
      // Was in a different channel — disconnect the old one
      existingSfu.disconnect().catch(() => undefined);
      setActiveSfuClient(null);
    }
    let cancelled = false;
    const sfu = new SfuClient(voiceChannel.id, user?.id);
          ...
          sfu
            ...
            .then(() => {
              ...
              return speakerSfu.connect().then(() => {
                ...
                setActiveSfuClient(speakerSfu);
                setVoiceJoined(true);
                setVoiceRequestPending(false);
                setLocalMicMuted(useVoiceStore.getState().isMuted);
              });
            })
            .catch((err) => {
  ...
  }, [voiceChannel?.id, voiceChannel?.isEnabled, currentGroup?.id, currentGroup?.name, voiceStoreSetActive, voiceStoreSetIsActive, user?.id]);

  if (!currentGroup) return <p className="p-6 opacity-70">CargandoΓÇª</p>;
  ...
  async function toggleChannel(channel: Channel, enabled: boolean) {
    ...
    try {
      ...
      // Realtime channel_updated event will refresh state ΓÇö no explicit reload
    } catch (error) {
  ...
  async function handleVoiceAction() {
    ...
    try {
      if (voiceJoined) {
        // Leave speaker mode ΓåÆ reconnect as passive listener
        await getActiveSfuClient()?.disconnect().catch(() => undefined);
        setActiveSfuClient(null);
        setVoiceJoined(false);
        setLocalMicMuted(true);
      ...
      if (!canManageChannels) {
        ...
        // Already approved ΓåÆ server joined us; fall through to set up the SFU client below.
      }
      await getActiveSfuClient()?.disconnect().catch(() => undefined);
      setActiveSfuClient(null);
      const sfu = new SfuClient(voiceChannel.id, user?.id);
  ...
  const sfuMicToggle = useCallback((muted: boolean) => {
    const activeSfu = getActiveSfuClient();
    if (!activeSfu || !voiceJoinedRef.current) {
      console.warn('[Voice] Cannot toggle mic from callback: SFU client not active or not joined as speaker.', { sfuRef: activeSfu, voiceJoined: voiceJoinedRef.current });
      return;
    }
    if (muted) {
      activeSfu.stopMic().catch((err: unknown) => console.error('[Voice] stopMic error', err));
    } else {
      activeSfu.publishMic().catch((err: unknown) => console.error('[Voice] publishMic error', err));
    }
  }, []);
  ...
  async function setMemberRole(memberUserId: string, role: GroupMember['role']) {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 404) {
        // User was kicked / is no longer a member ΓÇö remove from voice state
  ...
  async function moderateMember(memberUserId: string, action: 'BAN' | 'KICK' | 'PERMABAN' | 'UNBAN') {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 404) {
        // Already gone ΓÇö just clean up local state
        setVoiceParticipants((prev) => prev.filter((p) => p.id !== memberUserId));
        setPendingVoiceRequests((prev) => prev.filter((p) => p.id !== memberUserId));
        setSelectedMemberId(null);
        return;
  ...
  return (
    <div className="h-dvh max-w-[440px] w-full mx-auto bg-[#080a17] text-white flex flex-col overflow-hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      ...
      <div className="shrink-0 flex items-center gap-2.5 px-3 py-2.5 border-b border-white/[0.06] bg-[#0c0f1a]/95">
        ...
        <div className="min-w-0 flex-1">
          <div className="truncate text-[12px] font-semibold text-white/78">{currentGroup?.name ?? 'CargandoΓÇª'}</div>
          <div className="flex items-center gap-1.5 text-[10px] text-white/40 mt-0.5">
            ...
            <span className={presenceOnlineCount > 0 ? 'text-emerald-300/80' : ''}>{presenceOnlineCount} en l├¡nea</span>
          </div>
        </div>
      ...
      {/* ΓöÇΓöÇ Pending voice request approvals ΓöÇΓöÇ */}
        ...
        {showTextPanel && textChannel ? (
          ...
          <div className="relative flex h-full flex-col overflow-hidden bg-[#080a17]">
            <div className="flex flex-1 items-center justify-center px-6 text-center text-sm text-white/55">
              El chat de texto est├í apagado. Espera a que un admin o CoA lo encienda.
            </div>
            <div className="border-t border-white/8 bg-[#0e1021]/97 px-3 pb-3 pt-2">
              <div className="flex items-center gap-2">
                <div className="flex h-12 items-center gap-2 rounded-[16px] border border-white/8 bg-white/[0.03] px-3 text-sm text-white/60">
                  <MicMiniIcon />
                  <span>{voiceJoined ? 'Ya est├ís arriba en el chat de voz' : voiceRequestPending ? 'Solicitud enviada al admin o CoA' : 'Usa el bot├│n superior para subir a voz'}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {renderMemberPopup()}
      {/* ΓöÇΓöÇ Group Info Panel ΓöÇΓöÇ */}
      {activePanel === 'info' ? (
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 90, width: 'min(420px, 100vw)', background: '#0b0d1e', borderLeft: '1px solid rgba(255,255,255,.06)', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,.5)', animation: 'slideInRight .2s ease-out' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
              <span style={{ fontSize: 15, fontWeight: 600, color: '#f0f4ff' }}>Informaci├│n del grupo</span>
              <button onClick={() => setActivePanel(null)} style={{ width: 32, height: 32, borderRadius: '50%', border: 'none', background: 'rgba(255,255,255,.08)', color: '#f0f4ff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>Γ£ò</button>
            </div>
      ...
      {activePanel === 'settings' && canOpenSettings ? (
        <>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 90, width: 'min(420px, 100vw)', background: '#0b0d1e', borderLeft: '1px solid rgba(255,255,255,.06)', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,.5)', animation: 'slideInRight .2s ease-out' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,.06)', flexShrink: 0 }}>
              <div>
                <span style={{ fontSize: 15, fontWeight: 600, color: '#f0f4ff' }}>Configuraci├│n</span>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              ...
              {/* Description */}
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#727693', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 6 }}>Descripci├│n</div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <select value={groupForm.privacy} onChange={(e) => setGroupForm((c) => ({ ...c, privacy: e.target.value as GroupDetail['privacy'] }))} style={{ width: '100%', background: '#11142a', border: '1px solid rgba(255,255,255,.08)', borderRadius: 12, padding: '10px 14px', fontSize: 13, color: '#f0f4ff', outline: 'none', boxSizing: 'border-box' }}>
                  <option value="PUBLIC_INVITE">P├║blico con invitaci├│n</option>
              ...
              {/* ΓöÇΓöÇ Gesti├│n de miembros ΓöÇΓöÇ */}
              <div style={{ marginTop: 24, borderRadius: 18, border: '1px solid rgba(255,255,255,.06)', padding: 14, background: 'rgba(255,255,255,.02)' }}>
                <div style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#f0f4ff' }}>Gesti├│n de miembros</div>
                      ...
                      <div key={m.userId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 12, background: 'rgba(255,255,255,.03)' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 10, color: m.role === 'GROUP_ADMIN' ? '#f59e0b' : m.role === 'GROUP_MODERATOR' ? '#a78bfa' : '#727693' }}>
                            {m.role === 'GROUP_ADMIN' ? '≡ƒææ Admin' : m.role === 'GROUP_MODERATOR' ? '≡ƒ¢í∩╕Å CoA' : 'Miembro'}
                            {onlineUserIds.has(m.userId) ? <span style={{ marginLeft: 6, color: '#34d399' }}>ΓùÅ</span> : null}
                          </div>
                        </div>
              ...
              <div style={{ marginTop: 24, borderRadius: 18, border: '1px solid rgba(255,255,255,.06)', padding: 14, background: 'rgba(255,255,255,.02)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#f0f4ff' }}>Auditor├¡a</div>
                ...
                {!auditLoading && auditLogs.length === 0 ? <div style={{ padding: 12, color: '#727693', fontSize: 12 }}>A├║n no hay registros.</div> : null}
              ...
              <div style={{ marginTop: 28, borderRadius: 18, border: '1px solid rgba(239,68,68,.25)', padding: 16, background: 'rgba(239,68,68,.04)' }}>
                ...
                <div style={{ fontSize: 11, color: '#fca5a5', opacity: 0.7, marginBottom: 12 }}>Eliminar el grupo permanentemente. Esta acci├│n no se puede deshacer. Se borrar├ín todos los canales, mensajes, miembros, registros y archivos asociados.</div>
      ...
      {/* ΓöÇΓöÇ Banned Members Panel ΓöÇΓöÇ */}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                {(currentGroup?.members ?? []).filter((m) => m.isBanned).filter((m) => { const q = bannedSearch.toLowerCase().replace('@', ''); return !bannedSearch.trim() || m.user.displayName.toLowerCase().includes(q); }).length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 24, color: '#727693', fontSize: 13 }}>No hay miembros baneados{ bannedSearch ? ' que coincidan con la b├║squeda' : ''}.</div>
                ) : null}
              </div>
            </div>
          </div>
...
function formatAuditAction(entry: GroupAuditLog) {
  ...
  switch (entry.action) {
    case 'GROUP_UPDATED':
      return `${actor} edit├│ el grupo`;
    case 'MEMBER_JOINED':
      return `${target} entr├│ al grupo`;
    case 'MEMBER_KICK':
      return `${actor} expuls├│ a ${target}`;
    case 'MEMBER_BAN':
    case 'MEMBER_PERMABAN':
      return `${actor} expuls├│ a ${target}`;
    case 'MEMBER_UNBAN':
      return `${actor} rehabilit├│ a ${target}`;
    case 'MEMBER_ROLE_CHANGED':
      return `${actor} cambi├│ el rol de ${target}`;
    default:
      return entry.action;
  }
}
...
function formatAuditMeta(entry: GroupAuditLog) {
  ...
  if (entry.action === 'GROUP_UPDATED') {
    return 'Se guardaron cambios de nombre, descripci├│n, foto o privacidad.';
  }
