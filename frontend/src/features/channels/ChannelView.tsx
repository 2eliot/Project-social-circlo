...
function TextChannelView({ channel, minimal = false, showComposer = true, showVoiceControls = false, canToggleVoice = false, voiceEnabled = true, voiceBusy = false, onToggleVoice, canJoinVoice = false, voiceJoined = false, voiceJoinBusy = false, voiceRequestPending = false, onVoiceJoinAction, voiceChannelId, onMicMutedChange, memberRoles, canManageMembers = false, onToggleMemberMenu }: { channel: Channel; minimal?: boolean; showComposer?: boolean; showVoiceControls?: boolean; canToggleVoice?: boolean; voiceEnabled?: boolean; voiceBusy?: boolean; onToggleVoice?: () => void; canJoinVoice?: boolean; voiceJoined?: boolean; voiceJoinBusy?: boolean; voiceRequestPending?: boolean; onVoiceJoinAction?: () => void; voiceChannelId?: string; onMicMutedChange?: (muted: boolean) => void; memberRoles?: Record<string, GroupMemberRole>; canManageMembers?: boolean; onToggleMemberMenu?: (memberId: string) => void }) {
  ...
  useEffect(() => {
    ...
    const onPending = (m: Message) => setMessages((prev) => [...prev, m]);
  ...
  function send(e: React.FormEvent) {
    e.preventDefault();
    if (!showComposer || !input.trim()) return;
    const socket = getSocket('/chat');
          ...
          {previewMessages.length === 0 ? (
            ...
            return (
              <div key={`${message.id}-${message.createdAt}`} className={`flex ${mine ? 'justify-end pr-1' : 'justify-start pl-1'}`}>
                <div className={`relative max-w-[84%] rounded-[16px] border px-2.5 py-2 shadow-[0_10px_14px_rgba(0,0,0,.16)] ${mine ? 'border-white/10 bg-[rgba(47,51,61,.94)] text-white' : 'border-white/8 bg-[rgba(55,61,72,.92)] text-white/92'}`}>
                  <button
                    type="button"
                    disabled={!canOpenMemberMenu}
                    onClick={() => {
                  ...
                  >
                    <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-[#111925] text-[9px] font-black text-white/88">
                      {message.author?.avatarUrl ? <img src={message.author.avatarUrl} alt={authorLabel} className="h-full w-full object-cover" /> : authorLabel.slice(0, 2).toUpperCase()}
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 text-[10px] leading-[1.22]">{message.content}</div>
                    </div>
                  </button>
                </div>
              </div>
