              ...
              <div key={`${message.id}-${message.createdAt}`} className={`flex ${mine ? 'justify-end pr-1' : 'justify-start pl-1'}`}>
                <div className="flex max-w-[90%] items-center gap-2">
                  ...
                  >
                    ...
                    <div
                      className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-[#111925] text-[9px] font-black text-white/88 ${message.author?.id && !message.author?.isAnonymousProfile ? 'cursor-pointer' : ''}`}
                      onClick={(event) => {
                        if (!message.author?.id || message.author.isAnonymousProfile) return;
                        event.stopPropagation();
                        router.push(`/app?profileUserId=${encodeURIComponent(message.author.id)}`);
                      }}
                    >
